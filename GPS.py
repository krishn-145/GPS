...
print(f"\033[38;5;214m[{current_time}]\033[0m \033[1;32m[INFO]:\033[0m BY KRISHN OPEN")
...
parser.add_argument(
    '-p', '--port', type=int, default=8080, help='Web server port [ Default : http://localhost:8080 ]'
)
...
def banner():
    with open(META_FILE, 'r') as metadata:
        ...
        Telegram_url = json_data['Telegram']
        WEBSITE_url = json_data['WEBSITE']
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠸⡿⠿⠈⠐⠢⠤⠄"""
    ...
    utils.print(f'{G}[>] {C}Created By   : {G}hariJadhav')
    utils.print(f'{G} |---> {C}Telegram   : {G}{Telegram_url}')
    utils.print(f'{G} |---> {C}WEBSITE : {G}{WEBSITE_url}')
    utils.print(f'{G}[>] {C}Version      : {C}{VERSION}\n')
